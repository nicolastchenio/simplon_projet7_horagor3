...
================================
Configuration centralisée de Loguru pour la couche Présentation (frontend
Streamlit).

Copie auto-suffisante de ``src/observability/logging_config.py`` (voir
``mon_tutoriel.md``, section 8.2 : « un module logging_config.py par
service »). ``docker/frontend.Dockerfile`` copie tout ``src/`` dans
l'image, donc ce module importe ses constantes directement depuis
``src.config`` (pas de ``config.py`` frontend dédié) — exactement comme
``app_frontend.py`` le fait déjà pour ``API_BASE_URL``/``API_TIMEOUT``.

Différences avec la version Intelligence :

- Fichier de sortie ``frontend.log`` (au lieu de ``intelligence_api.log``),
  dans le même ``LOG_DIR`` partagé.
- Champ ``service`` fixé à ``"frontend"`` dans le JSON aplati.
- Aplatissement du record inliné ici (comme pour ``data_api``), plutôt
  que dans un module ``json_serializer.py`` séparé.
- Liste de loggers stdlib interceptés réduite à ``streamlit`` et
  ``httpx`` (le frontend n'a ni FastAPI ni Uvicorn).
"""
...
LOG_FILE_PATH: Path = LOG_DIR / "frontend.log"
...
def flatten_loguru_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    dupliqué ici pour rester autonome — seule différence : ``service``
    vaut ``"frontend"``.
...
class InterceptHandler(logging.Handler):
    ...
    par Streamlit/httpx et les ré-expédie dans le pipeline Loguru en
    conservant niveau, message et frame d'origine.
    """
...
def _configure_stdlib_interception() -> None:
    """
    Redirige les loggers stdlib du frontend vers Loguru via
    ``InterceptHandler`` (``streamlit``, ``httpx`` — pas de FastAPI ni
    d'Uvicorn côté Présentation).
    """
...
def setup_logging() -> None:
    """
    Configure le pipeline de logging Loguru pour le frontend Streamlit.

    Appeler **une seule fois par process** (voir la garde
    ``st.cache_resource`` dans ``app_frontend.py`` : Streamlit
    ré-exécute tout le script à chaque interaction utilisateur, il ne
    faut donc pas reconfigurer Loguru à chaque rerun).
    ...
    ).info("🚀 Loguru initialisé (frontend)")
